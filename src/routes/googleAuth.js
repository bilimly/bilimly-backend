...
router.get('/', (req, res) => {
  ...
  passport.use('google', new GoogleStrategy({
    clientID,
    clientSecret,
    callbackURL: 'https://bilimpark-backend-0zbt.onrender.com/api/auth/google/callback'
...
router.get('/callback', (req, res, next) => {
  const passport = require('passport');
  passport.authenticate('google', { session: false, failureRedirect: 'https://www.bilimpark.kg?error=auth' },
    (err, user) => {
      if (err || !user) return res.redirect('https://www.bilimpark.kg?error=auth');
